...
__global__ void ReduceSumWarpUnroll(int *input, int *temp, int size) {
	int tid = threadIdx.x;
...
int main() {
	...
	// Perform reduce sum with warp unroll on GPU
	GPUErrorCheck(cudaEventRecord(kernel_start, 0));
	ReduceSumWarpUnroll<<<grid, block>>>(device_input, device_temp, size);
